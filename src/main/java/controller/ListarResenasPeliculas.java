 ...
 * @author alexg
 */
public class ListarResenasPeliculas {
     public void mostrarResenas(JPanel jPanel) {
        ...
        for (Resenas resena : resenas) {
            PanelResenap panelResena = new PanelResenap(resena);
    ...
    private List<Resenas> obtenerResenasDesdeBD() {
        ...
        Query<Resenas> query = session.createQuery("FROM Resenas r JOIN FETCH r.peliculas", Resenas.class);
    ...
    private void llenarPanelConResena(PanelResenap panelResena, Resenas resena) {
        panelResena.getlblTitulo().setText(resena.getPeliculas().getTituloPelicula());
